...
- 先优先支持高频只读查询类能力，再逐步扩展结构化解析与配置类动作。
...
原因很简单：命令函数后面通常会带参数校验、命令拼装、返回解析和异常处理，完全配置化往往很快就不够用；而配置类操作又会引入配置模式切换、保存配置、失败回滚等更复杂的问题，适合后续独立推进。
...
第一阶段建议采用“厂商类 + 工厂函数”的轻量方案，先不要做得过重。
...
原因是操作层应该屏蔽厂商 CLI 差异，让调用方关注“我要什么信息”，而不是“这家厂商的命令长什么样”。
...
为了后面维护更顺，建议把命令函数按领域组织，而不是单纯按文件长度堆积：
...
但这一步不建议现在就做，因为当前项目体量还小，过早抽象容易让代码复杂度超过收益。
